...
*"The data is uni-modal, and right-skewed. The values are ranging from
641 to 2713, and the median is 1444, the mean is 1476. There are some
outliners above 2000. From the distribution of the data, the typical
area can be said using its median, thus 1444."*

```{r describe-sample, echo=TRUE}
# type your code for the Exercise here, and Knit

ggplot(data=samp, aes(x=area))+ geom_histogram(binwidth = 200,col="black")
```

1 True or False: My distribution should be similar to others'
distributions who also collect random samples from this population, but
it is likely not exactly the same since it's a random sample.
...
normally distributed and have standard error $s / \sqrt{n}$. Which of
the following is not a condition needed for this to be true?
...
```{r first-five-intervals, echo=TRUE}
ci %>%
  slice(1:5)
```
