...
2 Plot the relationship between `runs` and `at_bats`, using `at_bats` as
...
-   **The relationship is positive, linear, and moderately strong. One
...
-   **For each additional home run, the model predicts 1.83 more runs,
    on average.**
...
5 What is the residual for the prediction of runs for a team with 5,579
...
Predicted value is calculated as
follows;$$\hat{y} = 0.6305 \times{5579} -2789.2429 = 728.3166$$
...
#predicted value 
y_hat = 0.6305 *5579 -2789.2429 
#observed value
y_i = mlb11 %>% filter(at_bats == 5579) %>% select(runs)
...
-   **The residuals are fairly symmetric, with only a slightly longer
    tail on the right, hence it would be appropriate to deem the the
    normal distribution of residuals condition met.**

**Constant variability**:

9 Based on the residuals plot from earlier, the constant variability
...
strike_fit <- lm(runs~ strikeouts, data=mlb11)
summary(strike_fit)
...
ggplot(data=strike_fit, aes(x=.fitted, y=.resid))+
  geom_point()+
  geom_hline(yintercept = 0, linetype="dashed")

ggplot(data=strike_fit, aes(sample= .resid))+
  stat_qq()

#There is a negative correlation between "Strikeout" and "runs".
...
#as for at_bats R-squred is 0.3729, strikeouts is 0.1694
#comparing them, we conclude that "at_bat" is better than "strikeouts".
...
at_bats_rsqured <- summary(lm(runs~at_bats, data=mlb11))$r.squared
hits_rsquared <-summary(lm(runs~hits, data=mlb11))$r.squared
wins_rsquared <-summary(lm(runs~wins, data=mlb11))$r.squared
bat_avg_rsquared <-summary(lm(runs~bat_avg, data=mlb11))$r.squared
...
#bat_avg has the largest r-squared value. It predicts runs best. 
...
new_obs_rsquared <- summary(lm(runs~new_obs,data=mlb11))$r.squared
new_slug_rsquared <- summary(lm(runs~new_slug, data=mlb11))$r.squared
new_onbase_rsquared <- summary(lm(runs~new_onbase,data = mlb11))$r.squared

new_obs_rsquared
new_slug_rsquared
new_onbase_rsquared

#new_obs(on-base plus slugging) has the largest r-squared value. It predicts runs best. 
