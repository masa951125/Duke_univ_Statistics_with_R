...
can be accessed by typing `o09hhhv?nycflights?nycflights` in the
console.
...
2.  Make a histogram and calculate appropriate summary statistics for
    **arrival** delays of `sfo_feb_flights`. Which of the following is
    ...
    [No flight is delayed more than 2 hours.]{.underline}
...
ggplot(data=nycflights, aes(arr_delay)) +geom_histogram()
range(nycflights$arr_delay)
median(nycflights$arr_delay)
```
...
3.  Calculate the median and interquartile range for `arr_delay`s of
    ...
    [Delta and United Airlines]{.underline}
...
9.  Make a scatterplot of `avg_speed` vs. `distance`. Which of the
    ...
    [There is an overall postive association between distance and
    average speed.]{.underline}
...
nycflights %>% 
  mutate(avg_speed = distance/ air_time) %>%
  ggplot(aes(y=avg_speed, x= distance)) + geom_point()
```
...
mean(delayed_flights$arr_type=="on time")
tapply(nycflights$arr_type=="on time", nycflights$dep_type, mean)
#0.1833639
```
