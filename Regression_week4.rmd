...
summary(evals$score)
sum(evals$score<3)
ggplot(data=evals) + geom_histogram(aes(score))
```
...
#bty_avg, ethnicity
ggplot(data=evals, aes(x=ethnicity,y=bty_avg))+geom_boxplot()

#bty_avg, pic_color
ggplot(data=evals, aes(x=pic_color, y=bty_avg))+geom_boxplot()

#bty_avg, age
ggplot(data=evals, aes(x=age, y=bty_avg))+geom_point() + stat_smooth(method="lm")
...
```{r summary-model}
# type your code for the Exercise here, and Knit
bty_reg <-lm(score~ bty_avg, data=evals)
summary(bty_reg)
```

$$
equation: \hat{score} = 3.88034 + 0.06664\times{bty\text_{avg}} 
$$
...
```{r diag-score-bty_avg, warning=FALSE}
# type your code for the Question 5 here, and Knit

ggplot(data=bty_reg, aes(x=.fitted, y=.resid)) +geom_jitter() + 
  geom_hline(yintercept=0,col="red" )

ggplot(data=bty_reg, aes(.resid)) +geom_histogram()

ggplot(data=bty_reg, aes(x=seq(.resid), y=.resid)) +geom_jitter() + 
  geom_hline(yintercept=0,col="red" )
...
ggplot(data=evals, aes(x=gender, y=bty_avg)) +geom_boxplot()
```
...
```{r diag-mlr-model}
# type your code for the Exercise here, and Knit
m_bty_gen <-lm(score ~ gender+ bty_avg, data=evals)

ggplot(data=m_bty_gen, mapping=(aes(sample=.resid)))+ qqplotr::stat_qq_point()
ggplot(data=m_bty_gen) +geom_boxplot(aes(x=gender, y=.resid))
ggplot(data=m_bty_gen) +geom_jitter(aes(x=bty_avg, y=.resid))
```
...
p_val_select <- lm(score ~ rank + ethnicity + gender + language + age +
                     cls_perc_eval + cls_students + cls_level + cls_credits +
                     bty_avg + pic_outfit + pic_color, data = evals)
summary(p_val_select)
```
...
```{r adj-rsq-select, message=FALSE, warning=FALSE}
# type your code for the Exercise here, and Knit
...
::: {#license}
This is a product of OpenIntro that is released under a [Creative Commons Attribution-ShareAlike 3.0 Unported](http://creativecommons.org/licenses/by-sa/3.0). This lab was written by Mine Çetinkaya-Rundel and Andrew Bray.
:::
...
From this model, the would-be best scored professor is
...
-   proportion of students who had finished class evaluation is high

-   total number of student is larger the better

-   class level: upper

-   number of credits of class: one credit

-   beauty score: higher the better

-   picture : casual, black and white
