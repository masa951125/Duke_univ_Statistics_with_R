...
-   **False 50% of houses in Ames are smaller than 1,499.69 square
    feet.**
...
ggplot(data=samp1, aes(x= area)) + geom_histogram(binwidth=250, color="black")
...
ggplot(data=samp2, aes(x= area)) + geom_histogram(binwidth=250, color="black")
```
...
2 Suppose we took two more samples, one of size 100 and one of size
1000. Which would you think would provide a more accurate estimate of
the population mean?
...
sample_means_small <-ames %>% rep_sample_n(size=10, rep=25, replace = T)%>%
  summarise(x_bar = mean(area))
...
-   **TRUE Each element represents a mean square footage from a simple
    random sample of 10 houses.**
...
```{r shiny, eval=FALSE, message=TRUE, warning=TRUE, include=FALSE}
shinyApp(
  ui <- fluidPage(
...
5 It makes intuitive sense that as the sample size increases, the center
...
samp1 %>% summarize(x_bar_price=mean(price))
```
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_mean50 <- ames %>% rep_sample_n(size=50, rep=5000, replace=T) %>% 
  summarize(x_bar_price = mean(price))

ggplot(data=sample_mean50, aes(x=x_bar_price)) + geom_histogram(binwidth = 2500)
...
sample_mean150 <- ames %>% rep_sample_n(size=150, rep=5000, replace=T) %>% 
  summarize(x_bar_price = mean(price))

ggplot(data=sample_mean150, aes(x=x_bar_price)) + geom_histogram(binwidth = 2500) 
...
```{r}
sd(sample_mean50$x_bar_price)
sd(sample_mean150$x_bar_price)
...
sample_means_15 <- ames %>% rep_sample_n(size=15, rep=2000, replace=T) %>%
  summarize(x_bar_price = mean(price))

ggplot(data= sample_means_15, aes(x=x_bar_price)) + geom_histogram(binwidth = 2500)

print(mean(sample_means_15$x_bar_price))
print(sd(sample_means_15$x_bar_price))
print(mean(ames$price))
```
...
sample_means_150 <- ames %>% rep_sample_n(size=150, rep=2000, replace=T) %>%
  summarize(x_bar_price = mean(price))

ggplot(data= sample_means_150, aes(x=x_bar_price)) + 
  geom_histogram(binwidth = 2500)

print(mean(sample_means_150$x_bar_price))
print(sd(sample_means_150$x_bar_price))
print(mean(ames$price))
...
-   **FALSE The variability of the sampling distribution with the
